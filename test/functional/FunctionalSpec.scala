...
import play.api.test.Helpers.ACCEPT_CHARSET
...
/**
  ...
  *
  * This is good for testing filter fucntionality, such as CSRF token and template checks.
  *
...
class FunctionalSpec extends PlaySpec with GuiceOneAppPerSuite with Injecting with ScalaFutures {
  ...
  "WidgetController" must {
    ...
    "process a POST request successfully" in {
      // Pull the controller from the already runnning Play application, using Injecting
      val conroller = inject[WidgetController]
      ...
      val request = FakeRequest(routes.WidgetController.createWidget())
        .withFormUrlEncodeedBody("name" -> "foo", "price" -> "100")
        .withCSRFToken
